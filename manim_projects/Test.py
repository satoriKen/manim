...
class Test47(Scene):
    ...
    def construct(self):
        Test = VGroup()
        for i in range(51):
            test = TextMobject("\\Pisymbol{dancers}{%d}" % i, stroke_width=1, fill_opacity=1, stroke_opacity=1).scale(
                200)
            Test.add(test)
        self.wait()
...
class Test61(Scene):
    def construct(self):
        for1 = TexMobject(r"G(x)=\displaystyle\sum_{p=0}^{\infty}{\left( \frac{S^{p}(n)}{p!}x^p\right)}").scale(
            0.7).to_edge(UP + LEFT)
...
class Test74(Scene):
    ...
    def setup(self):
        ...
        tri_a = Polygon(self.A, np.array([1, -2, 0]), np.array([2.3, 1.3, 0]), color=RED, fill_color=RED,
                        fill_opacity=0.8)
        tri_b = Polygon(self.B, np.array([-2.3, 0.7, 0]), np.array([-1, -2, 0]), color=YELLOW, fill_color=YELLOW,
                        fill_opacity=0.8)
        tri_c = Polygon(self.C, np.array([1, 2.6, 0]), np.array([-1.6, 2, 0]), color=GREEN, fill_color=GREEN,
                        fill_opacity=0.8)
        self.tri = VGroup(tri_a, tri_b, tri_c)

        equation = TexMobject("S_{\\ } = S_{\\ } = S_{\\ } = S_{\\ }").scale(1.5).to_corner(UR, buff=1.1)
        tri_1 = self.main_tri.copy().set_stroke(width=0).set_fill(opacity=1).scale(0.2).next_to(equation[0][0],
                                                                                                RIGHT + DOWN * 3,
                                                                                                buff=-0.08)
        tri_2 = tri_a.copy().rotate(PI / 2).set_stroke(width=0).set_fill(opacity=1).scale(0.2).next_to(equation[0][2],
                                                                                                       RIGHT + DOWN * 3,
                                                                                                       buff=-0.08)
        tri_3 = tri_b.copy().rotate(PI / 2, axis=IN).set_stroke(width=0).set_fill(opacity=1).scale(0.2).next_to(
            equation[0][4], RIGHT + DOWN * 3, buff=-0.08)
        tri_4 = tri_c.copy().rotate(PI / 4, axis=IN).set_stroke(width=0).set_fill(opacity=1).scale(0.2).next_to(
            equation[0][6], RIGHT + DOWN * 3, buff=-0.08)
...
class Test90(Scene):
    ...
    def construct(self):
        ...
        C.add_updater(lambda m: m.move_to(circle.point_from_proportion(alpha.get_value())).flip(
            axis=B.get_center() - A.get_center(), about_point=ORIGIN))
...
class Test121(Scene):
    ...
    def construct(self):
        v = VectorizedPoint([1, 1, 0], stroke_width=10, stroke_opacity=1, fill_opacity=0, color=YELLOW,
                            artificial_width=10, artificial_height=10)
        self.add(v)
