...
class Test_MySectors(Scene):
    ...
    def construct(self):
        ...
        text_11 = Text('数据为3月14日前无新增确诊病例的27个省（市，区）', font='思源黑体 Bold', color=color).set_height(0.24).next_to(rect_1,
                                                                                                             RIGHT * 0.32).align_to(
            rect_1, DOWN)
        ...
        text_12 = Text('数据来源：@央视新闻', font='思源黑体 Bold', color=color).set_height(0.24).next_to(rect_2,
                                                                                             RIGHT * 0.32).align_to(
            rect_2, DOWN)
        ...
        text_13 = Text('作者：@cigar666', font='思源黑体 Bold', color=color).set_height(0.24).next_to(rect_3,
                                                                                               RIGHT * 0.32).align_to(
            rect_3, DOWN)
...
class Test_MySectors_0315(Scene):
    ...
    def construct(self):
        ...
        rect_1 = Rectangle(width=0.1, height=0.24, stroke_width=0, fill_color=color, fill_opacity=1)
        text_11 = Text('数据为3月15日前无新增确诊病例的25个省（市，区）', font='思源黑体 Bold', color=color).set_height(0.24).next_to(rect_1,
                                                                                                             RIGHT * 0.32).align_to(
            rect_1, DOWN)
...
class Test_Shadow(Scene):
    ...
    def construct(xgnb):
        ...
        shadow_02 = VGroup(
            *[Shadow_2d(text[2 * i], blur_width=0.5, layer_num=50, show_basic_shape=False) for i in range(5)]).shift(
            DOWN * 2)
...
class Test_Shadow_02(Scene):
    ...
    def construct(xgnb):
        ...
        shadows_out = VGroup(
            *[Shadow_2d(circles[i * 2], blur_width=0.5 * s, layer_num=50, show_basic_shape=False, shadow_out=True) for i
              in range(num)], plot_depth=1)
        shadows_in = VGroup(
            *[Shadow_2d(circles[i * 2 + 1], blur_width=0.5 * s, layer_num=50, show_basic_shape=False, shadow_out=False)
              for i in range(num - 1)], plot_depth=1)
        annulus_group = VGroup(*[
            Annulus(inner_radius=s * (2 * i - 1 + 0.6), outer_radius=s * (2 * i + 0.6), fill_color=colors[i],
                    fill_opacity=1, stroke_width=0, plot_depth=-5) for i in range(1, num)])
...
class Test_TransformMobject(Scene):
    ...
    def construct(self):
        ...
        hexagons = TransformMobject(hexagon, rotate_angle=-TAU, stroke_colors=WHITE, shift_vect=ORIGIN, num=100,
                                    rate_func=linear, scale_type=0)

        tri = RegularPolygon(3).set_width(2.).set_stroke(width=1.2).next_to(hexagon, UP, buff=0)
        tris = TransformMobject(tri, rotate_angle=2 * PI / 3, stroke_colors=WHITE, shift_vect=ORIGIN, num=40,
                                rate_func=linear, scale_type=0)
