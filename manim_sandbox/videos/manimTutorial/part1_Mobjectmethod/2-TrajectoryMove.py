...
class ShiftAndMoveto(Scene):
    ...
    def mob_moveto(self, mob, pos, run_time=1, start_stroke=10, fade_width=0.05, fade_opacity=0.01):
        ...
        n = int(dist / 0.1)
        for i in range(n):
            path = Line(mob.get_center(), mob.get_center() + vec / n, stroke_width=start_stroke, stroke_opacity=1,
                        stroke_color='#FFA500') \
                .add_updater(lambda p, dt: p.set_stroke(width=p.get_stroke_width() - fade_width,
                                                        opacity=p.get_stroke_opacity() - fade_opacity))
            self.add(path)
            self.play(
                mob.shift, vec / n,
                rate_func=linear,
    ...
    def construct(self):
        ...
        text = [
            Text(cap, font="Source Han Sans Bold", color=BLACK, plot_depth=2).scale(.5).to_edge(DOWN)
            for cap in captions
        ]
        ...
        svgmob = SVGMobject(r"mur.svg", color=BLACK) \
        ...
        text3 = VGroup(
            Text("mob.move_to(point_or_mobject,aligned_edge,coor_mask)", font="Source Han Sans Bold", color=BLACK,
                 plot_depth=2),
            Text("point_or_mobject是一个坐标或物体", font="Source Han Sans Bold", color=BLACK, plot_depth=2),
            Text("aligned_edge指与point_or_mobject对齐的边，默认为ORIGIN", font="Source Han Sans Bold", color=BLACK,
                 plot_depth=2),
