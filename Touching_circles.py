...
class Draw_touching_circles(ZoomedScene):
    ...
    def construct(self):
        ...
        dot_0 = Dot(ORIGIN, fill_color=RED, stroke_width=2.5, stroke_color=BLACK, plot_depth=2).scale(
            0.6)  # driving point
        dot_fix = Dot(DOWN * d, fill_color=PINK, stroke_width=2.5, stroke_color=BLACK, plot_depth=2).scale(
            0.6)  # fixed point
        ...
        def update_rect_group(rg):
            ...
            p_1 = complex_to_R3(d ** 2 / abs(R3_to_complex(p_0 - p_f)) * np.exp(
                (np.angle(R3_to_complex(p_0 - p_f)) + PI / 2) * 1j)) + p_f
            dot_1.become(Dot(p_1, fill_color=YELLOW, stroke_width=2.5, stroke_color=BLACK, plot_depth=2).scale(0.6))
            rect.become(Polygon(p_f, p_0, p_0 + p_1 - p_f, p_1, fill_color=BLUE, fill_opacity=0.4, stroke_width=4,
                                stroke_color=BLACK, plot_depth=0))
        ...
        def move_alone_circle(c, t0=1, t1=2):
            ...
            self.wait(t0 * 0.1)
            c0 = TracedPath(dot_0.get_center, strock_color=BLACK, stroke_width=3, min_distance_to_new_point=0.01,
                            plot_depth=0).add_updater(lambda c: c.set_color(BLACK))
            c1 = TracedPath(dot_1.get_center, strock_color=BLACK, stroke_width=3,
                            min_distance_to_new_point=0.01 * (i + 1) ** (-1) + 1e-4, plot_depth=0).add_updater(
                lambda c: c.set_color(BLACK))
...
class Transform_by_Inversion(MovingCameraScene):
    ...
    def construct(self):
        ...
        def get_circles(m, n, circle):
            return VGroup(
                *[VGroup(*[circle.copy().shift(UP * (i + 0.5) * d + RIGHT * (-(n - 1) / 2 + j) * d) for j in range(n)])
                  for i in range(m)])
        ...
        def get_inversed_circle(circle, z0, r):
            ...
            p1, p2 = center_old + complex_to_R3(r_old * np.exp(angle * 1j)), center_old - complex_to_R3(
                r_old * np.exp(angle * 1j))
        ...
        for i in range(m):
            ...
            self.wait(0.5)
            self.play(TransformFromCopy(circles[i], inv_circles[i]), circles[i].set_stroke, {'opacity': 0.5},
                      run_time=2)
